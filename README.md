...
- [GRDB Reference](http://groue.github.io/GRDB.swift/docs/2.9/index.html) (generated by [Jazzy](https://github.com/realm/jazzy))
...
1. [Download](https://github.com/groue/GRDB.swift/releases/tag/v2.9.0) a copy of GRDB, or clone its repository and make sure you use the latest tagged version with the `git checkout v2.9.0` command.
...
See [Configuration](http://groue.github.io/GRDB.swift/docs/2.9/Structs/Configuration.html) for more details.
...
See [Values](#values) for more information on supported arguments types (Bool, Int, String, Date, Swift enums, etc.), and [StatementArguments](http://groue.github.io/GRDB.swift/docs/2.9/Structs/StatementArguments.html) for a detailed documentation of SQLite arguments.
...
Values can be used as [statement arguments](http://groue.github.io/GRDB.swift/docs/2.9/Structs/StatementArguments.html):
...
Row adapters are values that adopt the [RowAdapter](http://groue.github.io/GRDB.swift/docs/2.9/Protocols/RowAdapter.html) protocol. You can implement your own custom adapters ([**:fire: EXPERIMENTAL**](#what-are-experimental-features)), or use one of the four built-in adapters, described below.
...
- `sqlite3_busy_handler`, `sqlite3_busy_timeout`: see [Configuration.busyMode](http://groue.github.io/GRDB.swift/docs/2.9/Structs/Configuration.html)
- `sqlite3_changes`, `sqlite3_total_changes`: see [Database.changesCount and Database.totalChangesCount](http://groue.github.io/GRDB.swift/docs/2.9/Classes/Database.html)
...
- `sqlite3_sql`: see [Statement.sql](http://groue.github.io/GRDB.swift/docs/2.9/Classes/Statement.html)
- `sqlite3_trace`: see [Configuration.trace](http://groue.github.io/GRDB.swift/docs/2.9/Structs/Configuration.html)
- `sqlite3_wal_checkpoint_v2`: see [DatabasePool.checkpoint](http://groue.github.io/GRDB.swift/docs/2.9/Classes/DatabasePool.html)
...
- [FetchableRecord] is able to **read**: it grants the ability to efficiently decode raw database row.
    ...
    Without FetchableRecord, you can decode rows yourself:
...
Raw SQL snippets are also accepted, with eventual [arguments](http://groue.github.io/GRDB.swift/docs/2.9/Structs/StatementArguments.html):
...
FTS3Pattern are regular [values](#values). You can use them as query [arguments](http://groue.github.io/GRDB.swift/docs/2.9/Structs/StatementArguments.html):
...
FTS5Pattern are regular [values](#values). You can use them as query [arguments](http://groue.github.io/GRDB.swift/docs/2.9/Structs/StatementArguments.html):
...
It is difficult to consume rows fetched from complex joined queries, because they often contain several columns with the same name: `id` from table `players`, `id` from table `teams`, etc.
...
```swift
    ...
    //        <------->
    let playersWidth = try db.columns(in: "players").count
    let playerAdapter = RangeRowAdapter(0 ..< playersWidth)
```

We also need an adapter that extracts teams' columns:
...
To avoid those problems, **never embed raw values in your SQL queries**. The only correct technique is to provide [arguments](http://groue.github.io/GRDB.swift/docs/2.9/Structs/StatementArguments.html) to your SQL queries:
...
Both DatabaseQueue and DatabasePool adopt the [DatabaseReader](http://groue.github.io/GRDB.swift/docs/2.9/Protocols/DatabaseReader.html) and [DatabaseWriter](http://groue.github.io/GRDB.swift/docs/2.9/Protocols/DatabaseWriter.html) protocols. DatabaseSnapshot adopts DatabaseReader only.
...
- Prepare to setup a [busy handler](https://www.sqlite.org/c3ref/busy_handler.html) with [Configuration.busyMode](http://groue.github.io/GRDB.swift/docs/2.9/Structs/Configuration.html)
