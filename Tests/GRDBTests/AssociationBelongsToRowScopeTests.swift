...
class AssociationBelongsToRowScopeTests: GRDBTestCase {
    ...
    func testDefaultScopeIncludingRequired() throws {
        ...
        XCTAssertEqual(rows[0].scopeNames, ["teams"])
        XCTAssertEqual(rows[0].scoped(on: "teams")!, ["id":1, "name":"Reds"])
    }
    ...
    func testDefaultScopeIncludingOptional() throws {
        ...
        XCTAssertEqual(rows[1].scopeNames, ["teams"])
        XCTAssertEqual(rows[1].scoped(on: "teams")!, ["id":nil, "name":nil])
    }
    ...
    func testCustomScopeIncludingRequired() throws {
        ...
        XCTAssertEqual(rows.count, 1)
        XCTAssertEqual(rows[0].unscoped, ["id":1, "teamId":1, "name":"Arthur"])
        XCTAssertEqual(rows[0].scopeNames, ["customTeam"])
        XCTAssertEqual(rows[0].scoped(on: "customTeam")!, ["id":1, "name":"Reds"])
    }
    ...
    func testCustomScopeIncludingOptional() throws {
        ...
        XCTAssertEqual(rows[0].scoped(on: "customTeam")!, ["id":1, "name":"Reds"])
        XCTAssertEqual(rows[1].unscoped, ["id":2, "teamId":nil, "name":"Barbara"])
        XCTAssertEqual(rows[1].scopeNames, ["customTeam"])
        XCTAssertEqual(rows[1].scoped(on: "customTeam")!, ["id":nil, "name":nil])
    }
