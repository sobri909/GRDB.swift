...
class HasOneAssociationTests: GRDBTestCase {
    ...
    func testSingleColumnNoForeignKeyNoPrimaryKey() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                let association = Parent.hasOne(Child.self, foreignKey: ["parentId"])
            ...
            do {
                let association = Parent.hasOne(Child.self, foreignKey: ["parentId"], to: ["id"])
                try assertEqualSQL(db, Parent.all().including(association), """
                    SELECT "parents".*, "children".* \
                    FROM "parents" \
    ...
    func testSingleColumnSeveralForeignKeys() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                let association = Parent.hasOne(Child.self, foreignKey: ["parent1Id"])
                try assertEqualSQL(db, Parent.all().including(association), """
                    SELECT "parents".*, "children".* \
                    FROM "parents" \
            ...
            do {
                let association = Parent.hasOne(Child.self, foreignKey: ["parent1Id"], to: ["id"])
                try assertEqualSQL(db, Parent.all().including(association), """
                    SELECT "parents".*, "children".* \
                    FROM "parents" \
                    JOIN "children" ON ("children"."parent1Id" = "parents"."id")
            ...
            do {
                let association = Parent.hasOne(Child.self, foreignKey: ["parent2Id"])
                try assertEqualSQL(db, Parent.all().including(association), """
                    SELECT "parents".*, "children".* \
                    FROM "parents" \
            ...
            do {
                let association = Parent.hasOne(Child.self, foreignKey: ["parent2Id"], to: ["id"])
                try assertEqualSQL(db, Parent.all().including(association), """
                    SELECT "parents".*, "children".* \
                    FROM "parents" \
                    JOIN "children" ON ("children"."parent2Id" = "parents"."id")
    ...
    func testCompoundColumnNoForeignKeyNoPrimaryKey() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                let association = Parent.hasOne(Child.self, foreignKey: ["parentA", "parentB"], to: ["a", "b"])
                try assertEqualSQL(db, Parent.all().including(association), """
                    SELECT "parents".*, "children".* \
                    FROM "parents" \
    ...
    func testCompoundColumnNoForeignKey() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                let association = Parent.hasOne(Child.self, foreignKey: ["parentA", "parentB"])
                try assertEqualSQL(db, Parent.all().including(association), """
                    SELECT "parents".*, "children".* \
                    FROM "parents" \
                    JOIN "children" ON (("children"."parentA" = "parents"."a") AND ("children"."parentB" = "parents"."b"))
    ...
    func testCompoundColumnSeveralForeignKeys() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                let association = Parent.hasOne(Child.self, foreignKey: ["parent1A", "parent1B"])
                try assertEqualSQL(db, Parent.all().including(association), """
                    SELECT "parents".*, "children".* \
                    FROM "parents" \
            ...
            do {
                let association = Parent.hasOne(Child.self, foreignKey: ["parent1A", "parent1B"], to: ["a", "b"])
                try assertEqualSQL(db, Parent.all().including(association), """
                    SELECT "parents".*, "children".* \
                    FROM "parents" \
                    JOIN "children" ON (("children"."parent1A" = "parents"."a") AND ("children"."parent1B" = "parents"."b"))
            ...
            do {
                let association = Parent.hasOne(Child.self, foreignKey: ["parent2A", "parent2B"])
                try assertEqualSQL(db, Parent.all().including(association), """
                    SELECT "parents".*, "children".* \
                    FROM "parents" \
            ...
            do {
                let association = Parent.hasOne(Child.self, foreignKey: ["parent2A", "parent2B"], to: ["a", "b"])
                try assertEqualSQL(db, Parent.all().including(association), """
                    SELECT "parents".*, "children".* \
                    FROM "parents" \
                    JOIN "children" ON (("children"."parent2A" = "parents"."a") AND ("children"."parent2B" = "parents"."b"))
