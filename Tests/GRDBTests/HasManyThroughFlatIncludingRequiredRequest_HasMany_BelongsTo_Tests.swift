...
class HasManyThroughFlatIncludingRequiredRequest_HasMany_BelongsTo_Tests: GRDBTestCase {
    ...
    func testSimplestRequest() throws {
        ...
        try dbQueue.inDatabase { db in
            let graph = try Country
                .flatIncluding(required: Country.citizens)
                .fetchAll(db)
    ...
    func testRightRequestDerivation() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                // filtered persons
                let graph = try Country
                    .flatIncluding(required: Country.citizens.filter(Column("name") != "Craig"))
                    .fetchAll(db)
            ...
            do {
                // ordered persons
                let graph = try Country
                    .flatIncluding(required: Country.citizens.order(Column("name").desc))
                    .fetchAll(db)
