...
class AssociationRowScopeSearchTests: GRDBTestCase {
    ...
    func testTestedRequest() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            XCTAssertEqual(row.scoped(on: "b")!.unscoped, ["id": 2, "aid": 1])
            XCTAssertEqual(row.scoped(on: "b")!.scoped(on: "c")!, ["id": 3, "aid": nil, "bid": 2])
            XCTAssertEqual(row.scoped(on: "b")!.scoped(on: "d")!, ["id": 5, "bid": 2])
            XCTAssertEqual(row.scoped(on: "c")!, ["id": 4, "aid": 1, "bid": nil])
        }
    }
    ...
    func testBreadthFirstScopeLookup() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            XCTAssertEqual(row.lookup(scope: "b")!.unscoped, ["id": 2, "aid": 1])
            XCTAssertEqual(row.lookup(scope: "c")!, ["id": 4, "aid": 1, "bid": nil])
            XCTAssertEqual(row.lookup(scope: "d")!, ["id": 5, "bid": 2])
        }
    }
