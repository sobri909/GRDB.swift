    ...
    func testRightAlias() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                ...
                let request = Country.including(optional: Country.continent.referenced(by: continentRef))
                try assertEqualSQL(db, request, """
                    SELECT "countries".*, "continents".* \
                    FROM "countries" \
                    LEFT JOIN "countryProfiles" ON ("countryProfiles"."countryCode" = "countries"."code") \
    ...
    func testLockedAlias() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                ...
                let request = Country.including(optional: Country.continent).referenced(by: countryRef)
                try assertEqualSQL(db, request, """
    ...
    func testConflictingAlias() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                ...
                let continentRef = TableReference(alias: "a")
                let request = Country.including(optional: Country.continent.referenced(by: continentRef)).referenced(by: countryRef)
