...
class BelongsToOptionalRequestTests: GRDBTestCase {
    ...
    func testRequest() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                ...
                let request = book.request(Book.optionalAuthor)
    ...
    func testFetchOne() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                let book = try Book.fetchOne(db, key: 1)!
                let author = try book.fetchOne(db, Book.optionalAuthor)
                assertEqualSQL(lastSQLQuery, "SELECT * FROM \"authors\" WHERE (\"id\" = 2)")
                assertMatch(author, ["id": 2, "name": "J. M. Coetzee", "birthYear": 1940])
            }
    ...
    func testRightAlias() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                // alias first
                let book = try Book.fetchOne(db, key: 1)!
                let request = book.request(Book.optionalAuthor.aliased("a"))
