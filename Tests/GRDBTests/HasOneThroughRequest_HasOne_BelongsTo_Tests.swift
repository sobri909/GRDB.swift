...
class HasOneThroughRequest_HasOne_BelongsTo_Tests: GRDBTestCase {
    ...
    func testRequest() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                ...
                let request = country.request(Country.continent)
    ...
    func testFetchOne() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                let country = try Country.fetchOne(db, key: "FR")!
                let continent = try country.fetchOne(db, Country.continent)
                assertEqualSQL(lastSQLQuery, """
                    SELECT "continents".* \
                    FROM "continents" \
                    JOIN "countryProfiles" ON (("countryProfiles"."continentId" = "continents"."id") AND ("countryProfiles"."countryCode" = 'FR'))
    ...
    func testRecursion() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                ...
                let rightAssociation = Person.belongsTo(Person.self, foreignKey: [Column("parentId")])
