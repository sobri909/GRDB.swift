...
class BelongsToJoinedOptionalRequestTests: GRDBTestCase {
    ...
    func testRightRequestDerivation() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                // filtered authors
                let graph = try Book
                    .joining(optional: Book.author.filter(Column("birthYear") >= 1900))
                    .fetchAll(db)
            ...
            do {
                // ordered books
                let graph = try Book
                    .joining(optional: Book.author.order(Column("name")))
                    .fetchAll(db)
    ...
    func testLockedAlias() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                ...
                let request = Book.joining(optional: Book.author).identified(by: bookRef)
                try assertEqualSQL(db, request, """
                    SELECT "AUTHORS".* \
                    FROM "books" "AUTHORS" \
            ...
            do {
                ...
                let request = Book.joining(optional: Book.author.identified(by: authorRef))
                try assertEqualSQL(db, request, """
                    SELECT "books1".* \
                    FROM "books" "books1" \
    ...
    func testConflictingAlias() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                let bookRef = TableReference(alias: "A")
                let authorRef = TableReference(alias: "a")
                let request = Book.joining(optional: Book.author.identified(by: authorRef)).identified(by: bookRef)
