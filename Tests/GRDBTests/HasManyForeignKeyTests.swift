...
class HasManyForeignKeyTests: GRDBTestCase {
    ...
    func testSingleColumnNoForeignKeyNoPrimaryKey() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                ...
                try assertEqualSQL(db, Parent.all().flatIncluding(required: association), """
                    SELECT "parents".*, "children".* \
                    FROM "parents" \
                    JOIN "children" ON ("children"."parentId" = "parents"."rowid")
                    """)
                try assertEqualSQL(db, Parent.all().flatIncluding(optional: association), """
                    SELECT "parents".*, "children".* \
                    FROM "parents" \
    ...
    func testForeignKeyDefinitionFromColumn() {
        ...
        struct Parent : TableMapping {
            ...
            static let children1 = hasMany(Child.self, using: Child.ForeignKeys.parent1)
            static let children2 = hasMany(Child.self, using: Child.ForeignKeys.parent2)
        }
