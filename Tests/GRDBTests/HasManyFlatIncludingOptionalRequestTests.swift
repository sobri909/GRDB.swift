...
class HasManyFlatIncludingOptionalRequestTests: GRDBTestCase {
    ...
    func testSimplestRequest() throws {
        ...
        try dbQueue.inDatabase { db in
            let graph = try Author
                .flatIncluding(optional: Author.books)
                .fetchAll(db)
    ...
    func testRightRequestDerivation() throws {
        ...
        try dbQueue.inDatabase { db in
            do {
                // filtered books
                let graph = try Author
                    .flatIncluding(optional: Author.books.filter(Column("year") < 2000))
                    .fetchAll(db)
            ...
            do {
                // ordered books
                let graph = try Author
                    .flatIncluding(optional: Author.books.order(Column("title")))
                    .fetchAll(db)
